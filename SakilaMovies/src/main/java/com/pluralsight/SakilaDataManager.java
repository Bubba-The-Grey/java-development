...
public class SakilaDataManager {
    ...
    public List<Movie> getMovieFromActor(String name){
        ...
        try(Connection connection = dataSource.getConnection(); PreparedStatement statement = connection.prepareStatement("select film.* from film " +
            ...
            try(ResultSet results = statement.executeQuery()){
                while(results.next()){
                    m.add(new Movie(results.getInt(1), results.getInt(4), results.getInt(9), results.getString(2), results.getString(3)));
                }
            }
            catch(SQLException e){
                System.out.println("Error occurred during the search");
    ...
    public List<Movie> getMovieFromActorID(String id){
        try(Connection connection = dataSource.getConnection(); PreparedStatement statement = connection.prepareStatement("select film.* from film " +
                "left join film_actor on film.film_id = film_actor.film_id " +
                "left join actor on film_actor.actor_id = actor.actor_id " +
